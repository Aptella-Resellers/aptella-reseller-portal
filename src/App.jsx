...
function ResellerForm({ onSaved }) {
  ...
  function validate() {
    ...
    if (keys.length) {
      ...
      const el = document.getElementById(idMap[keys[0]] || idMap.evidence);
      if (el) {
  ...
  async function submit(e) {
    ...
    // Convert selected files to data URLs and merge with pasted links
    ...
    const record = {
      ...
      notes: form.notes || "",
      evidenceLinks: allEvidenceLinks, // <— backend will attach these
      updates: [],
    };

    setSubmitting(true);
    try {
      const res = await gasPost("submit", record);
      if (!res.emailSent) {
        alert(
          "Saved, but email failed: " + (res.emailError || (res.attachErrors || []).join("; "))
        );
      } else if (res.attachErrors && res.attachErrors.length) {
        alert("Submitted. Some attachments were skipped: " + res.attachErrors.join("; "));
      } else {
...
function AdminPanel({ items, setItems, ratesAUD, setRatesAUD }) {
  ...
  async function doApprove(row) {
    const patch = { status: "approved", lockExpiry: addDays(todayLocalISO(), 60) };
...
/* ---------- AdminMap: safe Leaflet + optional clustering ---------- */
function AdminMap({ rows = [], ratesAUD = {}, height = 460 }) {
  ...
  const [usingCluster, setUsingCluster] = useState(false);

  const STATUS_META = {
    approved: { label: "Approved", color: "#16a34a" },
    pending: { label: "Pending", color: "#2563eb" },
    expiring: { label: "Expiring", color: "#f59e0b" },
    lost: { label: "Closed/Lost", color: "#dc2626" },
  };
  ...
  useEffect(() => {
    ...
    if (usingCluster && L.markerClusterGroup) {
      ...
      (rows || []).forEach((r) => {
        ...
        const html = `
          ...
          aud ? "(≈ AUD " + Math.round(aud).toLocaleString() + ")" : ""
        }</span></div>
    ...
    // Fallback non-cluster grid bubbles
    const group = L.layerGroup();
    layerRef.current = group;
    map.addLayer(group);

    const grid = new Map(); // key -> {lat,lng,total,count,items:[]}
    (rows || []).forEach((r) => {
      ...
      const aud = toAUD(r);
      if (!grid.has(key))
        grid.set(key, { lat: cellLat, lng: cellLng, total: 0, count: 0, items: [] });
